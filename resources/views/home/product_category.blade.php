				...
				<div class="content_box"><a href="{{asset('/products')}}/{{ $product->productCode }}">
				    @foreach($data['images'] as $image)
				     @if($product->productCode == $image->productCode && $image->class == 'primary')
			   	   	 <img style="width:100%;height:150px" src="{{asset('public/pimages')}}/{{$image->image}}" class="img-responsive" alt="">
			   	   	 @endif
			   	   	@endforeach
				   	  </a>
				    <h4><a href="{{asset('/products')}}/{{ $product->productCode }}">{{ $product->productName}}</a></h4>
				     <p>{{ substr($product->description,0,10) }}</p>
					 <div class="grid_1 simpleCart_shelfItem">
				    
					 <div class="item_add"><span class="item_price"><h6>{{ $product->price }} DA</h6></span></div>
					<div class="item_add"><span class="item_price"><a href="{{asset('products')}}/{{ $product->productCode }}/edit">Edit</a></span></div>
					<div class="item_add"><span class="item_price">
					{!! Form::open(['action' => ['ProductsController@destroy', $product->productCode], 'method' => 'POST']) !!}
